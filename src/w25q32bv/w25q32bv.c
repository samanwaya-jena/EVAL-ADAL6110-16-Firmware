...
static int w25q32bv_close(struct flash_info *fi)
{
	free(fi->erase_block_regions);
	return 0;
}

static int w25q32bv_write_enable(const struct flash_info *fi)
{
	return generic_write_enable (fi, WRITE_ENABLE);
}

static int w25q32bv_write_disable(const struct flash_info *fi)
{
	return generic_write_disable (fi, WRITE_DISABLE);
}

static int w25q32bv_read_mid_did(struct flash_info *fi, uint8_t *mid, uint8_t *did)
{
	uint8_t tbuf[10];
	uint8_t rbuf[2];
	uint8_t insn;
...
static int w25q32bv_read_uid(const struct flash_info *fi, uint64_t *uid)
{
	uint8_t tbuf[5];
	uint8_t rbuf[8];
	int count;
...
static int w25q32bv_read_jedec_id(const struct flash_info *fi, uint8_t *mid,
...
static int w25q32bv_read_status(const struct flash_info *fi, uint8_t *status, int n)
{
	uint8_t tbuf[1];
	uint8_t rbuf[1];
	int count;
...
static int w25q32bv_read_status1(const struct flash_info *fi, uint8_t *status)
{
	return w25q32bv_read_status(fi, status, 1);
}

static int w25q32bv_read_status2(const struct flash_info *fi, uint8_t *status)
{
	return w25q32bv_read_status(fi, status, 2);
}

static int w25q32bv_not_busy(const uint8_t status)
{
...
static int w25q32bv_write_status(const struct flash_info *fi, uint8_t status1, uint8_t status2)
{
	...
	w25q32bv_write_enable(fi);

	w25q32bv_read_status1(fi, &old_status1);
	...
	w25q32bv_wait_ready(fi);

	return 0;
}

static int w25q32bv_enable_quad_mode(const struct flash_info *fi)
{
	uint8_t status1, status2;

	w25q32bv_read_status1(fi, &status1);
	w25q32bv_read_status2(fi, &status2);
	...
	w25q32bv_write_status(fi, status1, status2);

	return 0;
}

static int w25q32bv_disable_quad_mode(const struct flash_info *fi)
{
	uint8_t status1, status2;
...
static int w25q32bv_read(const struct flash_info *fi, uint32_t addr, uint8_t *buf, int size)
{
	uint8_t read_insn;
	uint8_t tbuf[20];
	int count;
...
static int w25q32bv_erase_1(const struct flash_info *fi,
...
static int w25q32bv_erase(const struct flash_info *fi, uint32_t addr, int size)
{
	...
	while (size > 0)
	{
		w25q32bv_erase_1(fi, addr, ERASE_TYPE);
		addr += BLOCK_SIZE;
		size -= BLOCK_SIZE;
	}
...
static int w25q32bv_erase_chip(const struct flash_info *fi)
{
	return w25q32bv_erase_1(fi, 0, ERASE_CHIP);
}

static int w25q32bv_page_program(struct flash_info *fi,
...
static int w25q32bv_program(struct flash_info *fi,
		uint32_t addr, const uint8_t *buf, int size)
{
	int program_size;
	...
	while (size > 0)
	{
		...
		w25q32bv_page_program(fi, addr, buf, program_size);
...
static int w25q32bv_reset(const struct flash_info *fi)
{
	/* Not supported */
	return -1;
}

struct flash_info w25q32bv_info =
{
	"W25Q32BV",	/* name */
	"Winbond",	/* mname */
	...
	w25q32bv_read_mid_did,
	w25q32bv_read_uid,
	w25q32bv_read_jedec_id,
	w25q32bv_read_status1,
	w25q32bv_read_status2,
	w25q32bv_write_status,
	w25q32bv_enable_quad_mode,
	w25q32bv_disable_quad_mode,
	w25q32bv_read,
	w25q32bv_write_enable,
	w25q32bv_write_disable,
	w25q32bv_erase,
	w25q32bv_erase_chip,
	w25q32bv_program,
	w25q32bv_reset,
